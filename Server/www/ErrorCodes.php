...
class ErrorCodes {
    ...
    public function execute($client_id,$user_id,$request,$errorMessage,$errorFlags,$errorCode,$inputArray = null) {
        ...
        $errorMessage = $JSON->encode($errorMessage)?$JSON->encode($errorMessage) : $errorMessage;
        ...
        if (($errorFlags & ErrorCodes::$LOG_EMAIL) == ErrorCodes::$LOG_EMAIL) {
            $this->mail_utf8(Config::$error_email,'error','admin@admin.com','errorID:'.$errorCode,implode(" ",$msg));
        }
        ...
        $this->errorMessages[] = array("error"=>$errorMessage,"errorCode"=>$errorCode);

        return array("result"=>false,"error"=>$errorMessage,"errorCode"=>$errorCode);
    }
